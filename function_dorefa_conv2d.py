...
class DorefaConv2DFunction(function.Function):
    ...
    def forward_cpu(self, inputs):
        ...
        #Xq = quantize(self.col, E_a).astype(x.dtype, copy=False)
        W = numpy.clip(W*0.5+5, 0, 1)
        Wq = 2*quantize(W, E_w) - 1
        y = numpy.tensordot(
            self.col, Wq, ((1, 2, 3), (1, 2, 3))).astype(x.dtype, copy=False)
        if b is not None:
            y += b
    ...
    def forward_gpu(self, inputs):
        ...
        W_mat = cuda.cupy.clip(W_mat*0.5+0.5, 0, 1)
        
        col_mats = self.col.reshape(n, -1, out_h * out_w)
       
        W_mat = cuda.cupy.rint(W_mat*E_w)
        Wq_mat = _kern_w()(W_mat, E_w)

        y_mats = y.reshape(n, out_c, -1)
        # TODO(beam2d): Use streams or batch gemm
        for i in moves.range(n):
            y_mats[i] = Wq_mat.dot(col_mats[i])
        # TODO(beam2d): Support unshared bias
        if b is not None:
            y += b[:, None, None]
    ...
    def backward_gpu(self, inputs, grad_outputs):
        ...
        kh, kw = W.shape[2:]
        """
        N = cuda.cupy.random.uniform(-0.5, 0.5)


        coef = cuda.cupy.max(gy, axis=tuple([i for i in xrange(1,gy.ndim)])).astype(numpy.float32)
        coef = _as_mat(coef)
        gy = _as_mat(gy)
        coef_invert = 0.5*coef**(-1)
        gy = gy*coef_invert+0.5+N/E_g
        ...
        for i in moves.range(n):
            gcol_mats[i] = cuda.cupy.dot(Wq_mat.T, gy_mats[i])
...
def func_convolution_2d(x, W, b=None, stride=1, pad=0, use_cudnn=True,
    ...
    func = DorefaConv2DFunction(stride, pad, use_cudnn, cover_all)
    if b is None:
        return func(x, W)
    else:
        return func(x, W, b)
